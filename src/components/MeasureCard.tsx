  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {isSelected && (
          <span className="rounded-full bg-[var(--accent)]/15 px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--accent-strong)]">
            Active
          </span>
        )}
      </div>
      <input
        ...
        placeholder="e.g. Gm7 C7"
        aria-label={`Chords for bar ${measure.index + 1}`}
        className="w-full rounded-lg border border-transparent bg-transparent px-1 py-1 font-serif text-2xl text-[var(--foreground)] outline-none placeholder:text-[var(--muted)]/50 focus:border-[var(--accent)]/60 focus:bg-[var(--background)]/70"
      />
    </div>
  );
}
